## Dataverse Knowledge Integration
This demostrates uses Microsoft Dataverse tables (Account and Contact) as a knowledge source to answer business-related queries. It leverages Copilot Studio’s Knowledge feature to retrieve, interpret, and respond to structured organizational data with conversational context.

Using Copilot Studio’s Knowledge feature, Dataverse tables are connected directly to the copilot, enabling:
1.Structured data retrieval
2.Cross-table querying (Account and Contact)
3.Context-aware conversations
4.Business terminology understanding via column descriptions & glossary
5.Synonym support for flexible user queries
6.This implementation allows organizations to transform Dataverse data into an intelligent conversational assistant.

 ![Alt Text](https://github.com/bacdillon/Copilot-Studio-Agents/blob/main/Dataverse%20Knowledge%20Integration/Dataverse%20Knowledge.jpg)
